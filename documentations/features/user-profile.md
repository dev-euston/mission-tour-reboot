...
The identity surface for every user on the platform. A single user can hold any combination of the four roles — Player, Creator, Moderator, Business Owner — and each role runs as a separate, independent track on the profile. Tracks do not mix: player score does not affect creator tier, and so on.
...
Every signed-in user has a user profile, created automatically on first sign-up. Player and Creator tracks are active immediately. The Moderator and Business Owner tracks must be applied for and approved by a supervising Moderator before they appear on the profile.

The profile shows whichever role tracks are active for that user. A pure Player sees only their player track. A Creator who also plays sees both. A user running all four roles sees all four tracks.
...
| Active roles | Which of the four tracks are active for this user |

### Player Track
Visible if the user has ever started a mission.
...
### Creator Track
Visible if the user has ever saved a task or mission draft.
...
| Moderation standing | Current flag status (clean / flagged); visible only to the user |
...
### Moderator Track
Visible only if the user has been assigned Moderator status by a supervising Moderator.
...
Visible if the user has a linked business profile.
...
1. User sees a persistent "Apply for more roles" section on their profile listing the two gated roles (Moderator, Business Owner) with a brief description of each.
2. User taps to apply and is taken to the relevant application form.
3. On submission, the role shows as "Application pending" on the profile.
4. On approval or rejection, the user is notified on next page load.
5. Approved roles activate immediately and the new track appears on the profile.

### View Profile
1. User navigates to their profile from the nav.
...
2. When a threshold is crossed, the user sees an upgrade notification on next page load.
...
2. The user sees a notice on their creator track explaining the flag and what is required to lift it.
...
- **Multi-role** — tracks shown as tabs; user switches between them
...
- **Empty track** — a role with no activity yet shows a brief CTA (e.g. "Start your first mission" for a user who has never played)
...
- `UserProfile` — `id`, `handle`, `email`, `createdAt`, `isModerator`
- `PlayerProgression` — `userId`, `level`, `score`, `globalRank`
- `CreatorProgression` — `userId`, `tier`, `score`, `flagStatus`, `flagReason`
- `ModeratorProgression` — `userId`, `reviewCount`, `accuracyRate`
- `BusinessOwnerProgression` — `userId`, `businessProfileId`, `totalIssued`, `totalRedeemed`
- `UserBadge` — `userId`, `badgeId`, `earnedAt`
- `MissionStamp` — `userId`, `missionId`, `missionRunId`, `outcome`, `earnedAt`
